...
const workshops = (theme.value as any).workshops || []
...
const allCategories = computed(() => {
  ...
  workshops.forEach((workshop: any) => {
    const cats = workshop.frontMatter.categories || workshop.frontMatter.category
    if (!cats) return
...
const allTags = computed(() => {
  ...
  workshops.forEach((workshop: any) => {
    if (workshop.frontMatter.tags) {
      workshop.frontMatter.tags.forEach((tag: string) => tags.add(tag))
    }
  })
...
const filteredWorkshops = computed(() => {
  return workshops.filter((workshop: any) => {
    ...
    if (selectedCategories.value.length > 0) {
      ...
      const workshopCategories = Array.isArray(cats) ? cats : [cats]
      if (!workshopCategories.some(cat => selectedCategories.value.includes(cat))) {
        return false
      }
    }
    ...
    if (selectedTags.value.length > 0) {
      if (!workshop.frontMatter.tags || !workshop.frontMatter.tags.some((tag: string) => selectedTags.value.includes(tag))) {
        return false
      }
    }
  ...
  <ClientOnly>
    <div class="workshop-container">
      <div class="section-header">
        <div class="section-title">工坊</div>
        ...
        <div class="header-filters">
          ...
          <span class="page-stats">共 {{ filteredWorkshops.length }} 个</span>
        </div>
      </div>
      ...
      <div class="content-grid">
        <ModernCard
          ...
          :demo-url="workshop.frontMatter.demo"
          variant="workshop"
        />
      </div>
